...
#define WHICHFIELD FIELDJONMAD  //tom
...
void init_bondi_am()        //tom
{
    int i, j, k;
    double r, th, phi, ssth, cth;
    double ur, uh, up, u, rho;
    double X[NDIM];
    struct of_geom geom;
    double rhor,rb;
    double rc; //circularization radius
    double rho0,ur0;
    double sound, ra, lb, E;
    ...
    npow2 = 4.0; //power exponent
    cpow2 = 1.0; //exponent prefactor (the larger it is, the more hyperexponentiation is)
    rc = 0.0;    //circularizaton radius    //tom
    mdot = 2.0;    //tom
    ur0 = 0.3;  //tom

    t = 0.;
    hslope = 1.0; //uniform angular grid    //tom: I think this tells the information of the spherical grid
    ...
    DTd = 4.;	/* dumping frequency, in units of M */
    DTl = 4.;	/* logfile frequency, in units of M */
    DTi = 4.; 	/* image file frequ., in units of M */
    DTr = 400; /* restart file frequ., in units of M */
    DTr01 = 2000; /* restart file frequ., in timesteps */
...
/* start diagnostic counters */
    ...
    ZSLOOP(0, N1 - 1, 0, N2 - 1, 0, N3 - 1) {
        ...
        ssth = sin(sin(th));        //angular distrubution
        //cth = cos(th); //not use

        if (r > 2.0*rhor) {         //tom: the inner edge of the disk/ density distribution

            if (r < rc) {
                //ur = -sound * pow(r / ra, -0.5);
            ...
            //rho = RHOMINLIMIT + rho0 / pow(r / rhor, 1.5);      //*** 1.5   //tom: order
            rho = RHOMINLIMIT + mdot / (-4*M_PI*r*r*ur); //tom: not decide which one to use yet  //tom:16/8 forgot to put the minus sign, error
            //rho = 10.;
    ...
    ZSLOOP(0, N1, 0, N2, 0, N3) {
        ...
        else if (WHICHFIELD == FIELDJONMAD) {
            double JONMADHPOW, JONMADR0, JONMADRIN, JONMADROUT, rpow;
            JONMADHPOW = 4.0;
            JONMADR0 = -5.0;
            //JONMADR0 = -2.0; 
            JONMADRIN = 2.0; //inner edge I guess?
            JONMADROUT = 200; //jane: transition to monopolar field
            rpow = 1.0;
    ...
    else {      //Other fieldtype: FIELDJONMAD, OLEKFIELD, VERTICALFIELD, 
        /* finally, normalize to set field strength */  //nothing is changed
        beta_act = (gam - 1.) * umax / (0.5 * bsq_max);     //maybe need to change these    tom

        if (MASTER == mpi_rank) {
            fprintf(stderr, "initial beta: %g (should be %g)\n", beta_act, beta);
        }
        if (WHICH_FIELD_NORMALIZATION == NORMALIZE_FIELD_BY_BETAMIN) {
            beta_act = normalize_B_by_beta(beta, p, 80, &norm);    // normalised for r<80
        }
        else if (WHICH_FIELD_NORMALIZATION == NORMALIZE_FIELD_BY_MAX_RATIO) {
            beta_act = normalize_B_by_maxima_ratio(beta, p, &norm);
        }
