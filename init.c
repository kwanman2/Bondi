...
int init_grid(void)
{
  ...
  hslope = 1.0; //uniform angular grid  tom
...
int init_global(void)
{
  ...
  UUMIN = 1.7;
#elif(WHICHPROBLEM==BONDI)  
  BCtype[X1UP] = OUTFLOW;           //boundary condition
  BCtype[X1DN] = FREEOUTFLOW;
  rescaletype = 4;
  BSQORHOLIMIT = 50.;
  ...
  DTdumpgen[IMAGEDUMPTYPE] = 4.0;   //tom
...
int init_dsandvels(int inittype, int pos, int *whichvel, int*whichcoord, SFTYPE time, int i, int j, int k, FTYPE *pr, FTYPE *pstag)
{
  ...
  int init_dsandvels_bondi(int* whichvel, int* whichcoord, int i, int j, int k, FTYPE * pr, FTYPE * pstag);
...
#elif(WHICHPROBLEM==BONDI)
  return(init_dsandvels_bondi(whichvel, whichcoord, i, j, k, pr, pstag));           //tom
#endif
